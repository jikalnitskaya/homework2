...
$(function() {
	...
	var objNew = _.map(obj, "skills");
	console.log (objNew);
	//из массива массивов получаем один массив
	objNew = _.flatten(objNew);
	console.log (objNew);
	//убираем повторяющиеся значения
	objNew =_.uniq(objNew);
	console.log (objNew);
	objNew = _.toLower(_.forEach(objNew));
	console.log (objNew);
	objNew = _.split(objNew, ",");
	console.log (objNew);
    //сортируем по алфавиту
	objNew =_.sortBy(objNew, function(s){ 
        return s.charCodeAt();  
    });
    console.log (objNew);

    
    var objD =_.keyBy(obj, "name");
    console.log (objD);
    objD = _.mapValues(objD, function(o) { return _.size(o.friends); });
    console.log (objD);
    objD = _.invertBy(objD);
    console.log(objD);
    objD = _.flatten(_.values(objD));
    console.log (objD);


    var objM = _.map(obj, "friends");
	console.log (objM);
	objM = _.flatten(objM);
	console.log (objM);
	objM = _.uniq(_.map(objM, 'name'));
	console.log (objM);
